			  ...
			  can_tx_callback_t callback, void *callback_arg)
{
	...
	ret = mcp25xxfd_fifo_write(dev, MCP25XXFD_REG_FIFOCON(mailbox_idx + 1), &tx_frame,
				...
				const struct zcan_filter *filter)
{
	...
	if (filter_idx < CONFIG_CAN_MAX_FILTER) {
		...
		fltcon.FLTEN = 1;
		fltcon.FLTBP = MCP25XXFD_TXFIFOS + 1;
...
static uint32_t mcp25xxfd_tx_done(const struct device *dev)
{
	struct mcp25xxfd_data *dev_data = DEV_DATA(dev);
...
static void mcp25xxfd_int_thread(const struct device *dev)
{
	...
	while (1) {
		...
		while (1) {
			...
			emptied_mailboxes = intregs.ints.TXIF & (dev_data->mailbox_usage << 1);
			k_mutex_unlock(&dev_data->mutex);
			if (ret < 0) {
				continue;
			}
			...
			if (intregs.ints.RXIF) {
				mcp25xxfd_rx(dev, MCP25XXFD_TXFIFOS + 1);
			}

			if (intregs.ints.TEFIF) {
				emptied_mailboxes &= ~mcp25xxfd_tx_done(dev);
			}

			k_mutex_lock(&dev_data->mutex, K_FOREVER);
			emptied_mailboxes >>= 1;
			if (emptied_mailboxes) {
				LOG_DBG("TX Errors Detected: %x", emptied_mailboxes);
				for (int i = 0; i < MCP25XXFD_TXFIFOS; i++) {
					if (!(emptied_mailboxes & BIT(i))) {
						continue;
					}
					if (dev_data->mailbox[i].cb == NULL) {
						...
						dev_data->mailbox[i].cb(
							CAN_TX_ERR, dev_data->mailbox[i].cb_arg);
					}
...
static int mcp25xxfd_init(const struct device *dev)
{
	...
	txfifocon.FFIE = 1;
	for (int i = 0; i < MCP25XXFD_TXFIFOS; i++) {
		ret = mcp25xxfd_writew(dev, MCP25XXFD_REG_FIFOCON(i + 1), &txfifocon);
		if (ret < 0) {
			goto done;
		}
	}
...
#endif
	fifocon.FNEIE = 1;
	ret = mcp25xxfd_writew(dev, MCP25XXFD_REG_FIFOCON(MCP25XXFD_TXFIFOS + 1), &fifocon);
	if (ret < 0) {
		goto done;
	}
